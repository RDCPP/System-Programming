...
static void my_log(int priority, const char *fmt, ...);
...
main(int argc, char *argv[])
{
    ...
    if (daemon_mode) {
        ...
        /*
         * Syslog 로깅 API 중 openlog 함수를 사용하여 다음을 설정하세요.
         * 1. 로그에 출력되는 프로그램 이름
         * 2. 로그에 프로세스 ID 출력
         * 3. 로그의 범주를 daemon으로 설정
         **/
        become_daemon();
    }
...
become_daemon(void)
{
    ...
    /*
     * 이 곳에 데몬화 코드를 작성하세요.
     * 1. 프로세스의 current directory를 루트 디렉토리('/')로 변경
     * 2. 표준 입력, 출력, 에러를 /dev/null에 연결
     * 3. 세션 아이디 설정을 하여 제어 단말 제거
     *
     * 혹은 unistd.h의 daemon() 함수 사용.
     **/
}
...
server_main(int listen_sock_fd, char *docroot)
{
    for (;;) {
        ...
        accepted_sock_fd = accept(listen_sock_fd, (struct sockaddr*)&addr, &addrlen);
        if (accepted_sock_fd < 0) log_exit("accept(2) failed: %s", strerror(errno));

        struct sockaddr_in *sin;
        sin = (struct sockaddr_in *)&addr;
        my_log(LOG_INFO, "Request from %s:%d", inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));

        pid = fork();
        if (pid < 0) exit(3);
        if (pid == 0) {   /* child */
            FILE *inf = fdopen(accepted_sock_fd, "r");
            FILE *outf = fdopen(accepted_sock_fd, "w");
...
static void
my_log(int priority, const char *fmt, ...)
{
    va_list ap;
