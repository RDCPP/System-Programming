...
#define USAGE "Usage: %s [--port=n] [--daemon] <docroot>\n"
...
main(int argc, char *argv[])
{
    ...
    server_main(listen_sock_fd, docroot);
    exit(0);
}
...
detach_children(void)
{
	...
	/*
     * fork()된 자식 프로세스가 좀비화되는 것을 막기위해, 이 곳에 
     * sigaction()을 이용하여 이 프로세스는 더 이상 wait()을 안할
     * 것이라고 커널에 선언하는 코드를 구현하세요.
     *
     * 위에 선언된 변수를 활용하고, 필요 시 추가 가능합니다.
     *
     * 1. SIGCHLD와 함께 SA_NOCLDWAIT 플래그를 sigaction()에 전달
     * 2. 핸들러로 아무것도 하지않는 아래 noop_handler를 전달
     * 3. 이후 wait()을 수행하면 에러가 되기에 주의
     **/
}
...
listen_socket(char *port)
{
    ...
    /*
     * 이 함수에 socket()->bind()->listen() 과정을 구현하세요.
     * 위에 선언된 변수를 활용하세요.
     *
     * 1. getaddrinfo() 함수를 사용하여 프로세스가 동작하고 있는
     * 호스트의 addrinfo 구조체를 획득
     * 2. for loop 안에서 addrinfo 정보를 활용하여 socket()->
     * bind()->listen() 구현.
     * 3. 성공 시 return sockfd;
     **/
    ...
    for (ai = res; ai; ai = ai->ai_next) {
		...
		if ((sockfd = socket(ai->ai_family,ai->ai_socktype,ai->ai_protocol)) < 0) continue;
...
server_main(int listen_sock_fd, char *docroot)
{
    for (;;) {
        ...
        /*
         * 이 곳에 accept() 후 성립된 접속을 service()로 연결해주는
         * 코드를 작성하세요. 위에 선언된 변수를 활용하세요.
         *
         * fork()를 이용하여 복수 접속을 병렬 처리 하세요.
         * wait()는 하지 않습니다.
         *
         * 하나의 프로세스가 반복하여 accept()을 수행하고, 
         * service()는 fork()된 프로세스가 수행하도록 구현하세요.
         **/
    }
}
